...
def test_get_all_upcoming_classes(client):
    response = client.get('/api/classes/upcoming')  # Replace with your actual route
...
def test_booking_already_exists(client):
    ...
    client.post('/api/bookings', data=json.dumps(payload), content_type='application/json')
    response = client.post('/api/bookings', data=json.dumps(payload), content_type='application/json')
...
def test_get_bookings_by_email(client):
    ...
    response = client.get(f'/api/bookings/aditi@example.com')
